...
pub struct CoordinateTransform {
    pub _viewport_rect: egui::Rect,    // Current viewport bounds
    pub _document_rect: egui::Rect,    // Document bounds in document space  
    pub _scale: f32,                   // Current zoom scale
}

impl CoordinateTransform {
    pub fn new() -> Self {
        Self {
            _viewport_rect: egui::Rect::NOTHING,
            _document_rect: egui::Rect::from_min_size(egui::Pos2::ZERO, egui::Vec2::new(800.0, 600.0)),
            _scale: 1.0,
        }
    }
    ...
    pub fn screen_to_document(&self, screen_pos: egui::Pos2) -> egui::Pos2 {
        ...
        let relative_pos = screen_pos - self._viewport_rect.min;
        egui::pos2(relative_pos.x / self._scale, relative_pos.y / self._scale)
    }
    
    pub fn document_to_screen(&self, doc_pos: egui::Pos2) -> egui::Pos2 {
        // Convert document coordinates to viewport coordinates  
        let scaled_pos = egui::pos2(doc_pos.x * self._scale, doc_pos.y * self._scale);
        scaled_pos + self._viewport_rect.min.to_vec2()
    }
}
...
pub struct SpatialIndex {
    ...
    _dirty_regions: Vec<egui::Rect>,          // Regions needing re-render
}
...
impl SpatialIndex {
    ...
    pub fn mark_dirty_region(&mut self, bounds: egui::Rect) {
        self._dirty_regions.push(bounds);
    }
...
pub struct SpatialTextBuffer {
    ...
    pub _cursor_pos: usize,                   // Current cursor position in rope
    pub _selection: Option<(usize, usize)>,   // Selection range in rope
...
pub struct SpatialCursor {
    pub rope_pos: usize,
    pub _screen_pos: Option<egui::Pos2>,
    pub _blink_timer: std::time::Instant,
    pub _visible: bool,
}
...
impl SpatialCursor {
    pub fn new() -> Self {
        Self {
            rope_pos: 0,
            _screen_pos: None,
            _blink_timer: std::time::Instant::now(),
            _visible: true,
        }
    }
    
    pub fn update_position(&mut self, buffer: &SpatialTextBuffer) {
        self._screen_pos = buffer.rope_to_screen_position(self.rope_pos);
        
        // Update blink state
        if self._blink_timer.elapsed().as_millis() > 500 {
            self._visible = !self._visible;
            self._blink_timer = std::time::Instant::now();
        }
    }
    
    pub fn render(&self, painter: &egui::Painter) {
        if let Some(pos) = self._screen_pos {
            if self._visible {
    ...
    pub fn move_to_screen_position(&mut self, screen_pos: egui::Pos2, buffer: &SpatialTextBuffer) {
        if let Some(rope_pos) = buffer.screen_to_rope_position(screen_pos) {
            ...
            self._screen_pos = Some(screen_pos);
        }
    }
}