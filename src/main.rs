...
use egui_cosmic_text::widget::{CosmicEdit, Interactivity, HoverStrategy, FillWidth, NoContextMenu};
use rfd::FileDialog;
...
impl ChonkerApp {
    ...
    fn render_wysiwyg_readable(&mut self, ui: &mut egui::Ui) {
        ...
        if response.clicked() {
            if let Some(click_pos) = response.interact_pointer_pos() {
                ...
                let _doc_pos = self.spatial_buffer.viewport_to_document_transform.screen_to_document(click_pos);
                
                // Use sequential indexing to match cursor positioning logic
                let clicked_char_pos = {
                    let mut best_char = 0;
                    let mut best_distance = f32::MAX;
                    let mut current_char_index = 0;
                    
                    // Find closest glyph using sequential character counting
                    for run in self.text_buffer.layout_runs() {
                        for glyph in run.glyphs.iter() {
                            let glyph_screen_x = start_pos.x + glyph.x;
                            let glyph_screen_y = start_pos.y; // Same Y as rectangle rendering
                            let glyph_center_x = glyph_screen_x + 10.0; // Half of 20px width  
                            let glyph_center_y = glyph_screen_y + 12.0; // Half of 24px height
                            
                            let dx = click_pos.x - glyph_center_x;
                            let dy = click_pos.y - glyph_center_y;
                            let distance = (dx * dx + dy * dy).sqrt();
                            
                            if distance < best_distance {
                                best_distance = distance;
                                best_char = current_char_index; // Use sequential index, not glyph.start
                            }
                ...
                // Set cursor directly to the clicked character position - NO offset needed
                self.spatial_cursor.rope_pos = clicked_char_pos.min(self.spatial_buffer.rope.len_chars());
        ...
        let _cursor_char_in_line = cursor_pos - line_start;
        
        // Simple cursor positioning based on character count
        let cursor_screen_pos = {
            let rope = &self.spatial_buffer.rope;
            let cursor_pos = self.spatial_cursor.rope_pos.min(rope.len_chars());
            let line = rope.char_to_line(cursor_pos);
            let line_start = rope.line_to_char(line);
            let char_in_line = cursor_pos - line_start;
            
            let cursor_x = start_pos.x + (char_in_line as f32 * 8.4); // egui monospace width
            let cursor_y = start_pos.y + (line as f32 * 16.0); // line height
            
            egui::Pos2::new(cursor_x, cursor_y)
        };
        
        // Normal cursor size for text editing
        let cursor_rect = egui::Rect::from_min_size(
            cursor_screen_pos,
            egui::Vec2::new(2.0, 16.0) // Normal cursor width
        );
        ...
        // Skip cursor text for pure cosmic-text approach
                        ...
                        match key {
                            ...
                            egui::Key::C if i.modifiers.ctrl => {
                                ...
                                if let (Some(start), Some(end)) = (self.selection_start, self.selection_end) {
                                    ...
                                    let _selected_text = self.spatial_buffer.rope.slice(min_pos..max_pos).to_string();
    ...
    fn render_text_run_cosmic_static(painter: &egui::Painter, font_system: &mut cosmic_text::FontSystem, _swash_cache: &mut cosmic_text::SwashCache, text: &str, pos: egui::Pos2) {
        ...
        let mut run_buffer = cosmic_text::Buffer::new(font_system, cosmic_text::Metrics::new(14.0, 16.0));
        let attrs = cosmic_text::Attrs::new()
            .family(cosmic_text::Family::Name("SF Mono"))
            .weight(cosmic_text::Weight::NORMAL)
            .style(cosmic_text::Style::Normal);
        
        run_buffer.set_text(font_system, text, attrs, cosmic_text::Shaping::Advanced);
        run_buffer.shape_until_scroll(font_system, false);
        
        // Use cosmic-text to render actual glyphs
        for run in run_buffer.layout_runs() {
            for glyph in run.glyphs.iter() {
                let glyph_x = pos.x + glyph.x;
                let glyph_y = pos.y;
                
                // Render character using simple shapes - cosmic-text positioning, simple rendering
                if let Some(ch) = text.chars().nth(glyph.start) {
                    match ch {
                        ' ' => {
                            // Space - invisible
                        }
                        _ => {
                            ...
                            let char_rect = egui::Rect::from_min_size(
                                egui::Pos2::new(glyph_x, glyph_y),
                                egui::Vec2::new(6.0, 12.0)
                            );
                            painter.rect_filled(char_rect, 0.0, egui::Color32::WHITE);
                        }
                    }
                }
            }
    ...
    fn draw_simple_character_bitmap(&self, painter: &egui::Painter, ch: char, pos: egui::Pos2) {
        // Simple bitmap rendering for common characters
        // This maintains cosmic-text positioning while being readable
        let char_width = 8.0;
        let char_height = 12.0;
        
        match ch {
            'a'..='z' | 'A'..='Z' | '0'..='9' => {
                // For alphanumeric, draw a simple filled rectangle with the character
                let char_bg = egui::Rect::from_min_size(pos, egui::Vec2::new(char_width, char_height));
                painter.rect_filled(char_bg, 0.0, egui::Color32::from_gray(20));
            ...
            '.' => {
                ...
                let dot = egui::Rect::from_center_size(
                    egui::pos2(pos.x + char_width/2.0, pos.y + char_height - 2.0),
                    egui::Vec2::new(2.0, 2.0)
                );
                painter.rect_filled(dot, 1.0, egui::Color32::WHITE);
            }
            ',' => {
                // Comma
                let comma = egui::Rect::from_center_size(
                    egui::pos2(pos.x + char_width/2.0, pos.y + char_height - 1.0),
                    egui::Vec2::new(2.0, 3.0)
                );
                painter.rect_filled(comma, 1.0, egui::Color32::WHITE);
            }
            _ => {
                // Fallback: use egui text but at cosmic-text position
                painter.text(
                    pos,
                    egui::Align2::LEFT_TOP,
                    &ch.to_string(),
...
impl eframe::App for ChonkerApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // Resizable split - user can drag to adjust
        let screen_width = ctx.screen_rect().width();
        egui::SidePanel::left("xml_panel")
            .default_width(screen_width / 2.0)
        ...
        // Right panel: WYSIWYG Editor with scrolling
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.heading("📝 WYSIWYG Editor (Edit and Watch XML Update)");
            
            // Add scrollable area with cosmic text widget
            egui::ScrollArea::both()
                .auto_shrink([false, false])
                .show(ui, |ui| {
